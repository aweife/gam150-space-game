...
void Level1_Load()
{
	...
	Core::Get().AddComponent<cAI>(planet, new cAI);
}
...
void Level1_Init()
{
	...
	Core::Get().GetComponent<cRigidBody>(player)->velocityVector.x = 0.0f;
	Core::Get().GetComponent<cRigidBody>(player)->velocityVector.y = 0.0f;

	// Planet
	Core::Get().GetComponent<cTransform>(planet)->position.x = 0.0f;
	Core::Get().GetComponent<cTransform>(planet)->position.y = -200.0f;
	Core::Get().GetComponent<cRigidBody>(planet)->velocity = 0.0f;
	...
	Core::Get().GetComponent<cPathFinding>(planet)->target = player;
	Core::Get().GetComponent<cPathFinding>(planet)->currentState = PATH_SEEK;
	Core::Get().GetComponent<cAI>(planet)->minDistance = 250.0f;
	Core::Get().GetComponent<cAI>(planet)->maxDistance = 500.0f;

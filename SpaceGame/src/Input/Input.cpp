...
void inputUpdate()
{
	...
	float screenMousePosX = ((static_cast<float>(*pMousePosX) / (rect.right - rect.left))* (worldMaxX - worldMinX)
		- ((worldMaxX - worldMinX) / 2) - playerTransform->position.x);

	// Calculate mouse Y position in world space
	float screenMousePosY = -((static_cast<float>(*pMousePosY) / (rect.bottom - rect.top))* (worldMaxY - worldMinY)
		- ((worldMaxY - worldMinY) / 2)) - playerTransform->position.y;

	//printf("mousePosX = %f, mousePosY = %f \n", screenMousePosX, screenMousePosY);
	//printf("playerPosX = %f, playerPosY = %f", playerTransform->position.x, playerTransform->position.y);
	...
	if (AEInputCheckCurr(AEVK_UP))
	{
		...
		AEVec2Set(&playerDirection, AECos(playerTransform->rotation), AESin(playerTransform->rotation));
	...
	if (AEInputCheckCurr(AEVK_RBUTTON))
	{
		...
		AEVec2Set(&thrustDirection, AECos(playerTransform->rotation), AESin(playerTransform->rotation));
		// Bullet Speed
